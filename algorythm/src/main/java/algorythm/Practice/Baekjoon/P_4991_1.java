...
public class P_4991_1 {
    ...
    public static void main(String[] args) throws Exception{
        ...
        while (true) {
            ...
            List<Integer[]> perms = getPermutation(fastTimesMap.size() - 1);
            int minDist = Integer.MAX_VALUE;
            for (int i = 0; i < perms.size(); i++) {
                int dist = 0;
                Integer[] perm = perms.get(i);
    ...
    private static List<Integer[]> getPermutation(int size) {
        List<Integer[]> perms = new ArrayList<>();
        perm(perms, new Integer[size + 1], new boolean[size + 1], 1);
        return perms;
    }

    private static void perm(List<Integer[]> perms, Integer[] perm, boolean[] visited, int depth) {
        ...
        for (int i = 1; i < perm.length; i++) {
            if (!visited[i]) {
                visited[i] = true;
                perm[depth] = i;
                perm(perms, perm, visited, depth + 1);
                visited[i] = false;
            }
        }
    }
